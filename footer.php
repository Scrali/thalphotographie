...
$potentialMax = 0;
foreach ($items as $it) {
    if (substr($it['createdAt'] ?? '', 0, 7) === date('Y-m')) $month++;
    $potentialMin += (float)($it['price_min'] ?? 0);
    $potentialMax += (float)($it['price_max'] ?? 0);
}
?>
<h2>Estimations</h2>
...
<div class="panel-box">
  <p><a class="button small" href="estimation_settings.php">Modifier la tarification</a></p>
      ...
      <?php foreach ($items as $it): ?>
        <tr>
          ...
          <td class="actions-cell">
            <form method="post" action="estimation_convert.php">
